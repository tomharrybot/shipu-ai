module.exports = {
  config: {
    ...
    countDown: 30,
    role: 0,
    shortDescription: "🔥 Random 18+ video",
    longDescription: "Send a random hot 18+ video for you 😈",
    category: "18+",
    guide: "{p}sexvid",
  },
  ...
  onStart: async function ({ api, event, message }) {
    ...
    const loading = await message.reply({
      body: "🖤🦇 𝕎𝕒𝕚𝕥 𝕦𝕡, 𝕓𝒶𝒷𝓎... 𝕥𝕙𝕖 𝕟𝕚𝕘𝕙𝕥’𝕤 𝕙𝕠𝕥𝕥𝕖𝕤𝕥 𝕕𝕖𝕤𝕚𝕣𝕖𝕤 𝒶𝓇𝑒 𝕓𝕖𝕚𝕟𝕘 𝕦𝕟𝕝𝕖𝕒𝕤𝕙𝕖𝕕... 🥵💦",
    });

    const loadingMsgID = loading.messageID || loading?.messageReply?.messageID;
    ...
    const available = links.filter(l => !this.sentVideos.includes(l));
    if (available.length === 0) this.sentVideos = [];

    const vid = available[Math.floor(Math.random() * available.length)];
    this.sentVideos.push(vid);

    await message.reply({
      body: '💀👀 𝕐𝕠𝕦’𝕣𝕖 𝕕𝕖𝕖𝕡 𝕚𝕟 𝕥𝕙𝕖 𝕕𝕒𝕣𝕜𝕟𝕖𝕤𝕤, 𝕓𝕒𝕓𝕪... 🥵👄👅\n\n💦 𝕃𝕖𝕥 𝕞𝕪 𝕡𝕝𝕒𝕪𝕗𝕦𝕝 𝕝𝕚𝕡𝕤 𝕨𝕖𝕥 𝕪𝕠𝕦𝕣 𝕤𝕔𝕣𝕖𝕖𝕟 💋🫦💅',
